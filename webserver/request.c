...
uint32_t onserver(iserver_module *b, iserver_config *s)
{
 ...
 // do{  
    ...
    if(s->status != 100 && s->status != 200){
       ...
       ptr = request_pageLength(CCSHOW(request->ServerStatus), CCSHOW(request->ServerSoftware));
	   CCNEW(request->ContentLength , ptr);
       CLEAN(ptr);
    }else{
       if(!CCCHECK(request->ContentType)){
    ...
    if(CCSHOW(request->RequestMethod)==NULL){
     ...
     if(request_module_script(request, b, s->status)){
     	  
     	  #ifdef DEBUG_H
		  printf("Module Request (%s) \n", CCSHOW(request->PathTranslated));
     	  #endif
     	  
     	  csp_module_startup(&sapi_request, request);
     	  csp_module_execute(&sapi_request);
          ...
          if(sapi_request.buffer!=NULL){
     	  ...
     	  csp_module_cleanup(&sapi_request);
    ...
    // printf("> path (%s) \n", request->PathTranslated);
     ...
     if(cstrcmp(CCSHOW(request->RequestMethod),METHOD_GET_STR)==0){
          ...
          if(s->status != 100 && s->status != 200){
             ...
             size = request_readpage(buffer, 
                      CCSHOW(request->ServerStatus), 
          ...
          if(s->status == 100){
             ...
             request_dirinit();
             length = request_getfrontsize( CCSHOW(request->FileSelf));
             ...
             size = request_getfront(buffer, length, CCSHOW(request->FileSelf));
             b->cwrite(s, buffer, size );
             
             CLEAN(buffer);
             
             hfd = request_dirliststart(CCSHOW(request->PathTranslated));
             while((buffer=(char*)request_dirlistloop(&hfd,CCSHOW(request->FileSelf), &size))!=NULL){
                //printf("^%s \n", buffer);
                if(buffer!=NULL) {
				   if(b->cwrite!=NULL){
             ...
             CLEAN(buffer);
             request_dirlistend(&hfd);
           
             //printf(">>>>>>>>>>>>>>>>>>>>>>>>>\n");
             
             if(request->HTTPHostName==NULL){
             length = request_getbacksize( 
                               CCSHOW(request->ServerSoftware),  
             ...
             if(request->HTTPHostName==NULL){
             size = request_getback(buffer, length, 
                               CCSHOW(request->ServerSoftware),  
                               CCSHOW(request->ServerVersion),
                               CCSHOW(request->ServerOS),
                               CCSHOW(request->RemoteAddr),
