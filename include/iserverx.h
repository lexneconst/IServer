...
#define CLEAN(x)  if(x!=NULL) efree(x),x=NULL;
...
void *   SSL_Openssl_accept(iserver_config *server, SOCKET s_);
void     SSL_Openssl_close(iserver_config *server);
int      SSL_Recv(SSL *ssl,LPSTR buffer,unsigned int length);
int      SSL_Readbytes(SSL *ssl,LPSTR buffer,unsigned int length);
int      SSL_Write(SSL *ssl,LPSTR buffer,unsigned int length);
int      SSL_Writebytes(SSL *ssl,LPSTR buffer,unsigned int length);
unsigned 
