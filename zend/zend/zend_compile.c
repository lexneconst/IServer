...
static uint32_t zend_get_arg_num(zend_function *fn, zend_string *arg_name) {
	...
	if (fn->type == ZEND_USER_FUNCTION) {
		uint32_t i;
		for ( i = 0; i < fn->common.num_args; i++) {
...
#ifdef ZEND_WIN32
			ZSTR_LEN(dirname) = /*php_win32_ioutil_*/ zend_dirname(ZSTR_VAL(dirname), ZSTR_LEN(dirname));
#else
