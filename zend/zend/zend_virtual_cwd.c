...
void virtual_cwd_main_cwd_init(uint8_t reinit) /* {{{ */
{
	...
	result = /*php_win32_ioutil_*/ getcwd(cwd, sizeof(cwd));
#else
...
static inline zend_ulong realpath_cache_key(const char *path, size_t path_len) /* {{{ */
{
	...
	const char *bucket_key_start = /*tsrm_win32_*/ get_path_sid_key(path, path_len, &bucket_key_len);
...
retry_reparse_point:
		...
		if (save) {
			pathw = /*php_win32_ioutil_*/ any_to_w(path);
			if (!pathw) {
				return (size_t)-1;
			}
...
retry_reparse_tag_cloud:
		if(save &&
			...
			HANDLE hLink = NULL;
			WIN32_IOUTIL_REPARSE_DATA_BUFFER * pbuffer;
			DWORD retlength = 0;
			size_t bufindex = 0;
			...
			hLink = CreateFileW(pathw,
					0,
					WIN32_IOUTIL_DEFAULT_SHARE_MODE,
					NULL,
					OPEN_EXISTING,
			...
			pbuffer = (WIN32_IOUTIL_REPARSE_DATA_BUFFER *)do_alloca(MAXIMUM_REPARSE_DATA_BUFFER_SIZE, use_heap_large);
			if (pbuffer == NULL) {
				CloseHandle(hLink);
				free_alloca(tmp, use_heap);
				FREE_PATHW()
			...
			if(pbuffer->ReparseTag == IO_REPARSE_TAG_SYMLINK) {
...
#if VIRTUAL_CWD_DEBUG
				printname = /*php_win32_ioutil_*/ w_to_any(reparsetarget + pbuffer->MountPointReparseBuffer.PrintNameOffset  / sizeof(WCHAR));
				if (!printname) {
					free_alloca(pbuffer, use_heap_large);
					free_alloca(tmp, use_heap);
					FREE_PATHW()
				...
				substitutename = /*php_win32_cp_*/ conv_w_to_any(tmpsubstname, substitutename_len, &substitutename_len);
		...
		if (save && S_ISLNK(st.st_mode)) {
			...
			if (save) {
				size_t sz;
				char *tmp_path = /*php_win32_ioutil_*/ conv_w_to_any(dataw.cFileName, WIN32_CP_IGNORE_LEN, &sz);
				if (!tmp_path) {
					free_alloca(tmp, use_heap);
					FREE_PATHW()
					return (size_t)-1;
...
#ifdef ZEND_WIN32
	f = /*php_win32_ioutil_*/ fopen(new_state.cwd, mode);
#else
...
CWD_API int virtual_access(const char *pathname, int mode) /* {{{ */
{
	...
	ret = /*tsrm_win32_*/ access(new_state.cwd, mode);
#else
...
#ifdef ZEND_WIN32
	ret = /*win32_ */ utime(new_state.cwd, buf);
#else
...
CWD_API int virtual_chmod(const char *filename, mode_t mode) /* {{{ */
{
...
#ifdef ZEND_WIN32
	{
		...
		ret = /*php_win32_ioutil_*/ chmod(new_state.cwd, mode);
	}
#else
...
#ifdef ZEND_WIN32
		f = /*php_win32_ioutil_*/ open(new_state.cwd, flags, mode);
#else
		f = open(new_state.cwd, flags, mode);
#endif
	} else {
#ifdef ZEND_WIN32
		f = /*php_win32_ioutil_*/ open(new_state.cwd, flags);
#else
...
CWD_API int virtual_rename(const char *oldname, const char *newname) /* {{{ */
{
	...
	retval = /*php_win32_ioutil_*/ rename(oldname, newname);
#else
...
#ifdef ZEND_WIN32
	retval = /*php_win32_ioutil_*/ unlink(new_state.cwd);
#else
...
#ifdef ZEND_WIN32
	retval = /*php_win32_ioutil_*/ mkdir(new_state.cwd/*, mode*/);
#else
...
#ifdef ZEND_WIN32
	retval = /*php_win32_ioutil_*/ rmdir(new_state.cwd);
#else
