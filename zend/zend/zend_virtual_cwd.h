...
#  define MAXPATHLEN PATH_MAX //PHP_WIN32_IOUTIL_MAXPATHLEN
# elif PATH_MAX
...
#ifdef DEBUG_H
int win32_code_to_errno(unsigned long w32Err);

#define SET_ERRNO_FROM_WIN32_CODE(err) \
	do { \
	int ern = win32_code_to_errno(err); \
	SetLastError(err); \
	_set_errno(ern); \
	} while
#endif	
...
} WIN32_IOUTIL_REPARSE_DATA_BUFFER, *WIN32_IOUTIL_PREPARSE_DATA_BUFFER;

#define IOUTIL_FW_SLASHW L'/'
#define IOUTIL_FW_SLASH '/'
#define IOUTIL_BW_SLASHW L'\\'
#define IOUTIL_BW_SLASH '\\'
#define IOUTIL_DEFAULT_SLASHW IOUTIL_BW_SLASHW
#define IOUTIL_DEFAULT_SLASH IOUTIL_BW_SLASH

#define IOUTIL_DEFAULT_DIR_SEPARATORW	L';'
#define IOUTIL_IS_SLASHW(c) ((c) == IOUTIL_BW_SLASHW || (c) == IOUTIL_FW_SLASHW)
#define IOUTIL_IS_LETTERW(c) (((c) >= L'a' && (c) <= L'z') || ((c) >= L'A' && (c) <= L'Z'))
#define IOUTIL_JUNCTION_PREFIXW L"\\??\\"
#define IOUTIL_JUNCTION_PREFIX_LENW 4
#define IOUTIL_LONG_PATH_PREFIXW L"\\\\?\\"
#define IOUTIL_LONG_PATH_PREFIX_LENW 4
#define IOUTIL_UNC_PATH_PREFIXW L"\\\\?\\UNC\\"
#define IOUTIL_UNC_PATH_PREFIX_LENW 8

#define IOUTIL_IS_LONG_PATHW(pathw, path_lenw) (path_lenw >= IOUTIL_LONG_PATH_PREFIX_LENW \
	&& 0 == wcsncmp((pathw), IOUTIL_LONG_PATH_PREFIXW, IOUTIL_LONG_PATH_PREFIX_LENW))
#define IOUTIL_IS_UNC_PATHW(pathw, path_lenw) (path_lenw >= IOUTIL_UNC_PATH_PREFIX_LENW \
	&& 0 == wcsncmp((pathw), IOUTIL_UNC_PATH_PREFIXW, IOUTIL_UNC_PATH_PREFIX_LENW))
#define IOUTIL_IS_JUNCTION_PATHW(pathw, path_lenw) (path_lenw >= IOUTIL_JUNCTION_PREFIX_LENW \
	&& 0 == wcsncmp((pathw), IOUTIL_JUNCTION_PREFIXW, IOUTIL_JUNCTION_PREFIX_LENW))
#define IOUTIL_IS_ABSOLUTEW(pathw, path_lenw) (IOUTIL_IS_LONG_PATHW(pathw, path_lenw) \
	|| path_lenw >= 3 && IOUTIL_IS_LETTERW(pathw[0]) && L':' == pathw[1] && IOUTIL_IS_SLASHW(pathw[2]))
#define IOUTIL_IS_UNC(pathw, path_lenw) (path_lenw >= 2 && IOUTIL_IS_SLASHW(pathw[0]) && IOUTIL_IS_SLASHW(pathw[1]) \
	|| path_lenw >= IOUTIL_UNC_PATH_PREFIX_LENW && 0 == wcsncmp((pathw), IOUTIL_UNC_PATH_PREFIXW, IOUTIL_UNC_PATH_PREFIX_LENW))


#define WIN32_IOUTIL_DEFAULT_SHARE_MODE (FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE)
#define WIN32_CP_IGNORE_LEN (0)
#define WIN32_CP_IGNORE_LEN_P ((size_t *)-1)
...
static char *conv_w_to_any(const wchar_t* in, size_t in_len, size_t *out_len)
{/*{{{*/
	
}/*}}}*/

static wchar_t *any_to_w(const char* in)
{/*{{{*/
...
#ifdef ZEND_WIN32
# define php_sys_stat_ex stat_ex //php_win32_ioutil_stat_ex
# define php_sys_stat stat //php_win32_ioutil_stat
# define php_sys_lstat stat //php_win32_ioutil_lstat
# define php_sys_fstat fstat //php_win32_ioutil_fstat
# define php_sys_readlink readlink //php_win32_ioutil_readlink
# define php_sys_symlink symlink //php_win32_ioutil_symlink
# define php_sys_link link //php_win32_ioutil_link
#else
...
#if defined(ZEND_WIN32)
#define VCWD_FOPEN(path, mode)  /*php_win32_ioutil_*/ fopen(path, mode)
#define VCWD_OPEN(path, flags) /*php_win32_ioutil_*/ open(path, flags)
#define VCWD_OPEN_MODE(path, flags, mode) /*php_win32_ioutil_*/ open(path, flags, mode)
# define VCWD_RENAME(oldname, newname) /*php_win32_ioutil_*/ rename(oldname, newname)
#define VCWD_MKDIR(pathname, mode) /*php_win32_ioutil_*/ mkdir(pathname, mode)
#define VCWD_RMDIR(pathname) /*php_win32_ioutil_*/ rmdir(pathname)
#define VCWD_UNLINK(path) /*php_win32_ioutil_*/ unlink(path)
#define VCWD_CHDIR(path) /*php_win32_ioutil_*/ chdir(path)
#define VCWD_ACCESS(pathname, mode) /*tsrm_win32_*/ access(pathname, mode)
#define VCWD_GETCWD(buff, size) /*php_win32_ioutil_*/ getcwd(buff, size)
#define VCWD_CHMOD(path, mode) /*php_win32_ioutil_*/ chmod(path, mode)
#else
